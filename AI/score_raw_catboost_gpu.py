"""
CatBoost Site Scoring — GPU version for NVIDIA DGX Spark GB10.

Same pipeline as score_raw_catboost.py but runs on GPU.
CatBoost has native CUDA support — no RAPIDS needed.
Trains on GB10's Blackwell GPU for maximum speed.

Usage (on GB10):
    RAW_DIR=/home/acergn100_6/smart-city-management/data/raw python3 AI/score_raw_catboost_gpu.py

    # Force CPU if GPU issues:
    FORCE_CPU=1 python3 AI/score_raw_catboost_gpu.py
"""
import json
import os
import sys
    ...
    for name, y in [("energy", energy_y), ("waste", waste_y), ("nexus", nexus_y)]:
        ...
        if force_cpu:
            ...
            try:
                ...
                test_model.fit(Pool(X_train[:10], y_train[:10]))
                task_type = "GPU"
            except Exception:
                task_type = "CPU"

        if name == "energy":  # only print once
            print(f"\n  [CatBoost] {task_type} mode {'(GB10 Blackwell)' if task_type == 'GPU' else '(CPU fallback)'}\n")

        model = CatBoostRegressor(
            iterations=500,
            depth=6 if task_type == "GPU" else 4,  # GPU handles deeper trees efficiently
            learning_rate=0.1,
            l2_leaf_reg=5,
            subsample=0.8,
            loss_function="RMSE",
...
def save_results(sites_df, results, feature_cols):
    ...
    # Save
    for label, ranked, suffix in [("val", val_ranked, "catboost_gpu_val"), ("test", test_ranked, "catboost_gpu_test")]:
